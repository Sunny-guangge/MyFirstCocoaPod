...
- (id)initWithFrame:(CGRect)frame valueDictionary:(NSDictionary *)valueDictionary
{
    ...
    if (self) {
        ...
        _maxValue = 0;
        _valueDivider = 1;
        _drawboardColor = [UIColor blackColor];     //线框的颜色
...
- (void)calculateAllPoints
{
    ...
    NSArray *keyArray = [_valueDictionary allKeys];   //所有点的名字的集合
    NSArray *valueArray = [self getValueArrayFromDictionary:_valueDictionary keyArray:keyArray];//值
    _maxValue = [self getMaxValueFromValueArray:valueArray];//获取最大值
    NSArray *angleArray = [self getAngleArrayFromNumberOfSection:(int)[keyArray count]];//角度
...
- (NSArray *)getAngleArrayFromNumberOfSection:(int)numberOfSection//8
{
